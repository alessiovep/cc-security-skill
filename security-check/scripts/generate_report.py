...
<head>
    ...
    <title>Security Audit Rapport - {scan_date}</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="header">
            <h1>Security Audit Rapport</h1>
            <div class="meta">
                <div>Doel: <strong>{target}</strong></div>
                <div>Scandatum: <strong>{scan_date}</strong></div>
        ...
        <div class="footer">
            <p>Security Audit Rapport gegenereerd door Claude Security Skills</p>
            <p>Geautomatiseerde scanning kan fout-positieven opleveren. Handmatige review wordt aanbevolen.</p>
        </div>
    </div>
</body>
</html>"""
...
def _format_cwe_links(cwe_value) -> str:
    """Format CWE value(s) as clickable links. Handles string or list."""
    if not cwe_value:
        return ""
    cwe_ids = cwe_value if isinstance(cwe_value, list) else [cwe_value]
    links = []
    for cwe in cwe_ids:
        ...
        if match:
            ...
            links.append(
                f'<a class="cwe-link" href="https://cwe.mitre.org/data/definitions/{cwe_num}.html" '
                f'target="_blank" rel="noopener noreferrer">{cwe_str}</a>'
            )
        else:
            links.append(cwe_str)
    return ", ".join(links)


def _format_cve_links(cve_value) -> str:
    """Format CVE value(s) as clickable links. Handles string or list."""
    if not cve_value:
        return ""
    cve_ids = cve_value if isinstance(cve_value, list) else [cve_value]
    links = []
    for cve in cve_ids:
        cve_str = str(cve).strip()
        links.append(
            f'<a class="cwe-link" href="https://nvd.nist.gov/vuln/detail/{cve_str}" '
            f'target="_blank" rel="noopener noreferrer">{cve_str}</a>'
        )
    return ", ".join(links)
...
def generate_html_report(json_report_path: str, output_path: str = None):
    ...
    if not vulnerabilities_html:
        vulnerabilities_html = '''
        <div class="section">
            <h2>Geen kwetsbaarheden gevonden</h2>
            <p>De security scan heeft geen kwetsbaarheden ge&iuml;dentificeerd in de gescande codebase.</p>
            <p>Dit garandeert niet de afwezigheid van alle beveiligingsproblemen. Handmatige security review wordt nog steeds aanbevolen.</p>
        </div>'''

    summary = data.get('summary', {})
    html_content = HTML_TEMPLATE.format(
        target=data.get('target', 'Onbekend'),
        scan_date=data.get('scan_date', 'Onbekend'),
    ...
    print(f"HTML rapport gegenereerd: {output_path}")
    return output_path


def main():
    if len(sys.argv) < 2:
        print("Gebruik: python generate_report.py <json_rapport_pad> [output_html_pad]")
        sys.exit(1)
